...
/*
 * Modified from {@link org.elasticsearch.memcached.netty.NettyMemcachedServerTransport}
 */
...
import org.elasticsearch.redis.RedisServerTransport;
...
/**
 */
public class NettyRedisServerTransport extends AbstractLifecycleComponent<RedisServerTransport> implements RedisServerTransport {
    ...
    protected void doStart() throws ElasticsearchException {
        ...
        if (blockingServer) {
            serverBootstrap = new ServerBootstrap(new OioServerSocketChannelFactory(
                    Executors.newCachedThreadPool(daemonThreadFactory(settings, "redis_server_boss")),
                    Executors.newCachedThreadPool(daemonThreadFactory(settings, "redis_server_worker"))
            ));
        } else {
        ...
        ChannelPipelineFactory pipelineFactory = new ChannelPipelineFactory() {
            ...
            public ChannelPipeline getPipeline() throws Exception {
                ...
                pipeline.addLast("decoder", new RedisDecoder(logger));
                pipeline.addLast("dispatcher", new RedisDispatcher(restController));
                return pipeline;
            }
        };
